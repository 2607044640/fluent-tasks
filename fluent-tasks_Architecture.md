...
- **Sync Field Backward Compatibility**: Optional fields `dueDate?`, `msGraphId?`, `msGraphListId?` on `TaskItem` MUST remain optional and NEVER be required. (Why: existing users without sync must not be affected — these fields only appear in `%%{...}%%` metadata when populated by `A1MSTodoSync`).
