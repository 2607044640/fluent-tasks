...
- **Sync Field Backward Compatibility**: Optional fields `dueDate?`, `msGraphId?`, `msGraphListId?`, `frames?` on `TaskItem` MUST remain optional and NEVER be required. (Why: existing users without sync must not be affected — these fields only appear in `%%{...}%%` metadata when populated).
...
- **Image Frames Lucide Rendering**: Icons in visual memory frames MUST use Obsidian native `setIcon(node, iconName)` API to guarantee light/dark theme conformity and zero external bundle overhead.
</key_invariants>
