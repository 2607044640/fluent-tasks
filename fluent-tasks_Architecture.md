...
- **Quick List Grid Board & 2D Spatial Physics Protocol**: `QuickListModalView` defaults to a multi-column Grid Board layout (`quickListGridLayout: true`), rendering root categories and groups as cards across full screen width (`repeat(auto-fill, minmax(280px, 1fr))`) so users can see all lists without scrolling. Arrow keys execute 2D vector navigation (`navigateDirection`) based on DOM element centers (`dx` / `dy` scoring) with graceful column wrapping, preserving cursor movement when search input text is actively edited. Users can toggle back to classic single-column list mode via settings or an in-modal header toggle.
