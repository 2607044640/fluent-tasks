...
- **Quick List Modal (`open-quick-list-modal`)**: A dedicated list-only floating navigator conforming to the **A1 Floating UI Standard** (`88vw × 86vh`, `12px` rounded corners, elevated depth shadows, smooth backdrop blur, and click-outside dismissal). Tiles all groups and lists across multi-column cards like a dashboard chart (Grid Board layout, enabled by default), making every list instantly visible across the expansive canvas without unnecessary scrolling. Features full 2D geometric keyboard navigation (<kbd>↑↓←→</kbd> move across lists and cards, <kbd>Enter</kbd> open, <kbd>F2</kbd> inline rename, <kbd>Esc</kbd> close), Chinese IME composition protection, lossless typing redirection from anywhere in the modal into the search input, drag-and-drop reordering, an in-modal header `[Grid / List]` toggle, an integrated `[⛶ Maximize]` toggle to switch between A1 floating and 100vw full-screen modes, and an integrated top-bar `[✕ Close]` button. Selecting a list reveals the center task view, collapses open sidebars for a distraction-free view, and suppresses sidebar auto-expansion.
