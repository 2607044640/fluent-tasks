...
- **Quick List Modal (`open-quick-list-modal`)**: A dedicated list-only floating navigator that tiles all groups and lists across multi-column cards like a dashboard chart (Grid Board layout, enabled by default), making every list instantly visible across the full screen without unnecessary scrolling. Features full 2D geometric keyboard navigation (<kbd>↑↓←→</kbd> move across lists and cards, <kbd>Enter</kbd> open, <kbd>F2</kbd> inline rename, <kbd>Esc</kbd> close), real-time search filtering, drag-and-drop reordering, and a settings switch with an in-modal header toggle (`Grid / List`) to switch between Grid Board and classic single-column list views. Selecting a list reveals the center task view, collapses open sidebars for a distraction-free view, and suppresses sidebar auto-expansion.
